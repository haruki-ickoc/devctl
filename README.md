...
- **Makefile の共通化・吸収 (`devctl run`)**: 各リポジトリに散乱しがちな `make migrate` や `make seed` などのタスクを設定ファイル (`config.yaml`) に集約。
...
│   ├── config/                  # 設定ローダー、YAML パース、チルダ/環境変数展開
│   │   ├── config.go
│   │   └── template.go
...
devctl project up web-app    # または devctl up web-app

# プロジェクトを停止
devctl project down web-app  # または devctl down web-app

# プロジェクトのログを確認
devctl project logs -f web-app

# コンテナ内でコマンドを実行
devctl project exec web-app app sh

# 設定ファイルに定義されたカスタムタスク（旧 Makefile 処理）を実行
devctl run web-app migrate
devctl run web-app seed
```

> **カレントディレクトリの自動判定**:
> プロジェクトの作業ディレクトリに移動している場合、`devctl up` や `devctl run migrate` のようにプロジェクト名の引数を省略して実行できます。
